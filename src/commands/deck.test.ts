...
vi.mock(import("../lib/config.ts"), async (importOriginal) => {
  ...
  return { ...actual, loadConfig: vi.fn<typeof actual.loadConfig>() };
});
...
describe(deckCli, () => {
  it("loads config, parses flags, and hands off to the runner", async () => {
    loadConfigMock.mockResolvedValue(makeConfig(4567));
    ...
    expect(input?.config.deck).toStrictEqual({ port: 4567 });
